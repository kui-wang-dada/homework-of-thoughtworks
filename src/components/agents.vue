        ...
        <div class="agentsMain">
            <div class="leftContent">
                ...
                <div v-show="isShow" class="modalAgent">
                    ...
                    <div class="modal_btn">
                        <input type="button" value="Add agent" @click.stop="addAgent()">
                        <input type="button" value="Close" @click.stop="cancelAgent()">
                    </div>
                </div>
                <ul>
                    <li v-for="(item,index) in recommends" @click="addHistory(item)" :class="item.isChoose" :key=index>
                        ...
                        <div class="deny" v-if="item.title.typelist[0]!=='building'">
                            <i class="icon iconfont icon-forbidden"></i>Deny
                        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getStyle() {
                this.recommends.forEach((ele, index) => {
                    if (ele.title.typelist[0] === "idle") {
                        this.Summary[1].count += 1
                        ele.isChoose = "idle"
                    } else {
                        this.Summary[0].count += 1
                        ele.isChoose = "building"
                    }
                })
            },
            ...
            addResource(item) {
                ...
                let arr = item.inputMsg.split(",");
                let arrNew = [].concat(item.resources, arr);
            ...
            addAgent() {
                let recommendNew = {
                    listId: this.recommends.length ,
                    isShow: false,
                    isChoose: "idle",
                    inputMsg: "",
                    title: {
                        name: this.modalInputName,
                        typelist: [
                            "idle",
                            this.modalInputIP,
                ...
                this.recommends.push(recommendNew)
                this.Summary[1].count+=1
                this.isShow=false
            },
            cancelAgent() {
